// =============================================================================
// INSTRUCTOR PAGE WITH SEO
// Replace: app/(dashboard)/(routes)/instructor/[instructorId]/page.tsx
// =============================================================================
...
export async function generateMetadata({
  ...
  const { data: instructorRaw } = await supabase
    .from("profiles")
    .select("name, title, bio, image_url")
    .eq("user_id", params.instructorId)
    .single();

  const instructor = instructorRaw as unknown as {
    name: string;
    title: string | null;
    bio: string | null;
  ...
  const title = `${instructor.name} - Instructor`;
  const description =
    instructor.bio?.slice(0, 160) ||
    `${instructor.name} is an instructor at Mezzo Aid. ${instructor.title || ""}`;
  const imageUrl = instructor.image_url || siteConfig.ogImage;
  ...
  return {
    ...
    alternates: {
      canonical: `/instructor/${params.instructorId}`,
    },
    openGraph: {
      type: "profile",
      title: `${title} | Mezzo Aid`,
      description,
      url: `${siteConfig.url}/instructor/${params.instructorId}`,
...
const InstructorPage = async ({ params }: InstructorPageProps) => {
  ...
  if (courses.length === 0) {
    // No courses, return early with SEO
    return renderPage(instructorProfile, [], 0, 0, user, params.instructorId);
  }
  ...
  // Map courses with their chapters and purchase status
...
// Extracted render logic with SEO
function renderPage(
  ...
  const instructorSchema = generateInstructorSchema({
    id: instructorId,
    name: instructorProfile.name,
    imageUrl: instructorProfile.image_url,
    title: instructorProfile.title,
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    {
      name: instructorProfile.name,
      url: `${siteConfig.url}/instructor/${instructorId}`,
    },
  ]);
  ...
  return (
    <>
      ...
      <JsonLd data={[instructorSchema, breadcrumbSchema]} />
